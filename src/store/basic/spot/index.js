...
// ** Fetch Spots
export const fetchSpot = createAsyncThunk('spot/fetchData', async (params, thunkAPI) => {
  const header = {
...
export const addSpot = createAsyncThunk('spot/addData', async (params, thunkAPI) => {
  const header = {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${window.localStorage.getItem(urls.access)}`
  }
...
export const editSpot = createAsyncThunk('spot/editData', async (params, thunkAPI) => {
  const header = {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${window.localStorage.getItem(urls.access)}`
  }
...
export const deleteSpot = createAsyncThunk('spot/deleteData', async (params, thunkAPI) => {
  const header = {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${window.localStorage.getItem(urls.access)}`
  }
...
export const spotSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(editSpot.fulfilled, (state, action) => {
        state.data = action.payload?.items
        state.total = action.payload?.total
      })
